...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def has_permission(self, permission):
        ...
        if isinstance(collection_permission, collections.abc.Callable):
...
class CollectionHandler(BaseHandler):
    ...
    def get(self, id_=None, resource=None, resource_id=None, acl=True, **kwargs):
        if resource:
            ...
            if handler and isinstance(handler, collections.abc.Callable):
...
class LoginHandler(RootHandler):
    ...
    def get(self, **kwargs):
        ...
        if self.is_api():
            ...
            # serve the main Angular app which will handle the login route
            with open(os.path.join(os.path.dirname(__file__), 'angular_app', 'index.html'), 'r', encoding='utf-8') as f:
                self.write(f.read())
...
class RegisterHandler(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        # register a new user
        try:
            username = self.get_body_argument('new-username')
            email = self.get_body_argument('new-email')
            password = self.get_body_argument('new-password')
        except tornado.web.MissingArgumentError:
            data = escape.json_decode(self.request.body or '{}')
        ...
        if not (username and email and password):
            self.set_status(400)
            self.write({'error': True, 'message': 'missing required fields'})
            return
        
        # Check if user already exists
        existing_user = self.db.query('users', {'username': username})
        if existing_user:
            self.set_status(400)
            self.write({'error': True, 'message': 'username already exists'})
            return
        
        # Check if email already exists
        existing_email = self.db.query('users', {'email': email})
        if existing_email:
            self.set_status(400)
            self.write({'error': True, 'message': 'email already exists'})
            return
        ...
        try:
            ...
            logging.info('successful registration for user %s (id: %s)' % (username, user_id))
            self.write({'error': False, 'message': 'registration successful', 'redirect': '/login'})
        except Exception as e:
            logging.error('registration failed for user %s: %s' % (username, str(e)))
            self.set_status(500)
...
def run():
    ...
    application = tornado.web.Application([
            ...
            (r"/landing", LandingHandler, init_params),
            ...
            (r'/register', RegisterHandler, init_params),
            (r'/v%s/register' % API_VERSION, RegisterHandler, init_params),
