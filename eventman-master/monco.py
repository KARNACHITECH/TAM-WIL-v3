...
class Monco(object):
    ...
    def update(self, collection, _id_or_query, data, operation='update',
        ...
        existing_doc = db[collection].find_one(_id_or_query)
        updated_existing = existing_doc is not None
        
        res = db[collection].find_one_and_update(filter=_id_or_query,
                update={operator: data}, return_document=ReturnDocument.AFTER, upsert=create)
        return updated_existing, res or {}
